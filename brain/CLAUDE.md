...
6. **Models vary per agent** — Quality-critical agents (Email Responder, Main, Morning Briefing, Evening Wind-Down, Overnight PR, Failure Analyzer, Improvement Analyst) use Sonnet 4.6. All other agents use GLM-5. Fallback: GLM-5 → MiniMax M2.5 → Gemini 2.5 Pro. Local Ollama models (qwen3:14b) are only for Python maintenance scripts — not for agentic tool-calling.
