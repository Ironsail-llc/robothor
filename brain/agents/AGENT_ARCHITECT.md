...
- **Satisfaction rate**: `get_agent_stats()` now includes `satisfaction_rate` and `outcome_distribution` (successful/partial/incorrect/abandoned). An agent with a low satisfaction_rate (many "partial" or "incorrect" outcomes) should be weighted higher than one with simple timeouts — incorrect results are worse than failures.
...
- **Infrastructure health:** Read `watchdog_log` and `autodream_log` blocks. Look for patterns: recurring PG/Redis failures (infra instability), zombie runs being reaped (agents stalling), autodream staleness (memory consolidation not running). These are infra problems that degrade all agents — flag them as `needs-philip` structural tasks if persistent.
