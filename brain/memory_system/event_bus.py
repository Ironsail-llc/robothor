...
Usage:
    from event_bus import publish, subscribe, ack
...
def _get_redis():
    ...
    try:
        ...
        # Read URL lazily to avoid pollution from other modules' dotenv loading
    ...
    *,
    ...
    if agent_id is not None:
        try:
            ...
            if not capabilities.check_stream_access(agent_id, stream, "write"):
                logger.warning(
    ...
    try:
        ...
        msg_id = r.xadd(key, envelope, maxlen=MAXLEN, approximate=True)
        return msg_id
    except Exception as e:
    ...
    *,
    ...
    if agent_id is not None:
        try:
            ...
            if not capabilities.check_stream_access(agent_id, stream, "read"):
                logger.warning(
...
def stream_length(stream: str) -> int:
    ...
    try:
        ...
        if r is None:
            return 0
        return r.xlen(_stream_key(stream))
    except Exception as e:
