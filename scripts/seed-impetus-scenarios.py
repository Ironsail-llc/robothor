...
def sql(query):
    ...
    cmd = ["sudo", "docker", "exec", "impetus_one_postgres", "psql", "-U", "impetus", "-d", "impetus_rx", "-c", query]
    result = subprocess.run(cmd, capture_output=True, text=True)
...
if maria:
    ...
    rx1 = api_post("/api/prescriptions", {
    ...
    if rx1:
        sql(f"UPDATE prescriptions SET status='transmitted', prescribed_at='{days_ago(45)}', transmitted_at='{days_ago(44)}', transmission_method='scriptsure' WHERE id='{rx1}';")

    # Rx 2: Alprazolam (Schedule IV) — draft, awaiting provider signature
    rx2 = api_post("/api/prescriptions", {
        "patient": f"/api/patients/{maria}",
        "provider": f"/api/providers/{PROVIDERS['sarah_johnson']}",
    ...
    rx3 = api_post("/api/prescriptions", {
        "patient": f"/api/patients/{maria}",
        "provider": f"/api/providers/{PROVIDERS['sarah_johnson']}",
    ...
    if rx3:
        sql(f"UPDATE prescriptions SET status='filled', prescribed_at='{days_ago(60)}', transmitted_at='{days_ago(59)}', filled_at='{days_ago(58)}', transmission_method='unknown' WHERE id='{rx3}';")

    # Rx 4: Hydrocodone (Schedule II) — FAILED transmission, needs attention
    rx4 = api_post("/api/prescriptions", {
        "patient": f"/api/patients/{maria}",
        "provider": f"/api/providers/{PROVIDERS['sarah_johnson']}",
    ...
    if rx4:
        sql(f"UPDATE prescriptions SET status='failed', prescribed_at='{days_ago(2)}', transmission_method='rxvortex' WHERE id='{rx4}';")

    # Appointment: Follow-up in 3 days
    apt1 = api_post("/api/appointments", {
        "patient": f"/api/patients/{maria}",
        "provider": f"/api/providers/{PROVIDERS['sarah_johnson']}",
...
if david:
    ...
    rx5 = api_post("/api/prescriptions", {
    ...
    if rx5:
        sql(f"UPDATE prescriptions SET status='transmitted', prescribed_at='{days_ago(180)}', transmitted_at='{days_ago(179)}', transmission_method='scriptsure' WHERE id='{rx5}';")

    # Rx 2: Atorvastatin — transmitted
    rx6 = api_post("/api/prescriptions", {
        "patient": f"/api/patients/{david}",
        "provider": f"/api/providers/{PROVIDERS['michael_chen']}",
    ...
    if rx6:
        sql(f"UPDATE prescriptions SET status='transmitted', prescribed_at='{days_ago(180)}', transmitted_at='{days_ago(179)}', transmission_method='scriptsure' WHERE id='{rx6}';")

    # Rx 3: Post-op prednisone taper — FAILED (pharmacy system down), URGENT
    rx7 = api_post("/api/prescriptions", {
        "patient": f"/api/patients/{david}",
        "provider": f"/api/providers/{PROVIDERS['michael_chen']}",
    ...
    if rx7:
        sql(f"UPDATE prescriptions SET status='failed', prescribed_at='{days_ago(1)}', transmission_method='dispensepro' WHERE id='{rx7}';")

    # Appointment: Today, in-person post-op check — confirmed
    apt2 = api_post("/api/appointments", {
        "patient": f"/api/patients/{david}",
        "provider": f"/api/providers/{PROVIDERS['michael_chen']}",
        "clinic": f"/api/clinics/{CLINICS['downtown']}",
    ...
    apt3 = api_post("/api/appointments", {
        "patient": f"/api/patients/{david}",
...
catherine = api_post("/api/patients", {
    "firstName": "Catherine",
    "lastName": "Shaw",
    "dob": "1948-11-30",
    "gender": "female",
...
if catherine:
    ...
    rx8 = api_post("/api/prescriptions", {
    ...
    if rx8:
        sql(f"UPDATE prescriptions SET status='transmitted', prescribed_at='{days_ago(90)}', transmitted_at='{days_ago(89)}', transmission_method='unknown' WHERE id='{rx8}';")

    # Rx 2: Amlodipine (add-on for BP) — transmitted
    rx9 = api_post("/api/prescriptions", {
        "patient": f"/api/patients/{catherine}",
        "provider": f"/api/providers/{PROVIDERS['lisa_cuddy']}",
    ...
    if rx9:
        sql(f"UPDATE prescriptions SET status='transmitted', prescribed_at='{days_ago(60)}', transmitted_at='{days_ago(59)}', transmission_method='unknown' WHERE id='{rx9}';")

    # Rx 3: Metformin — dose increase, newly prescribed
    rx10 = api_post("/api/prescriptions", {
        "patient": f"/api/patients/{catherine}",
    ...
    if rx10:
        sql(f"UPDATE prescriptions SET status='signed', prescribed_at='{days_ago(1)}', signed_by_id='{PROVIDERS['james_wilson']}', signed_by_name='Dr. James Wilson' WHERE id='{rx10}';")

    # Rx 4: Levothyroxine — stable, filled
    rx11 = api_post("/api/prescriptions", {
        "patient": f"/api/patients/{catherine}",
        "provider": f"/api/providers/{PROVIDERS['lisa_cuddy']}",
    ...
    if rx11:
        sql(f"UPDATE prescriptions SET status='filled', prescribed_at='{days_ago(30)}', transmitted_at='{days_ago(29)}', filled_at='{days_ago(27)}', transmission_method='unknown' WHERE id='{rx11}';")

    # Rx 5: Atorvastatin — needs refill soon (draft for renewal)
    rx12 = api_post("/api/prescriptions", {
        "patient": f"/api/patients/{catherine}",
        "provider": f"/api/providers/{PROVIDERS['lisa_cuddy']}",
    ...
    apt4 = api_post("/api/appointments", {
        "patient": f"/api/patients/{catherine}",
        "provider": f"/api/providers/{PROVIDERS['james_wilson']}",
        "clinic": f"/api/clinics/{CLINICS['downtown']}",
    ...
    apt5 = api_post("/api/appointments", {
        "patient": f"/api/patients/{catherine}",
        "provider": f"/api/providers/{PROVIDERS['lisa_cuddy']}",
        "clinic": f"/api/clinics/{CLINICS['westside']}",
...
if marcus:
    ...
    rx13 = api_post("/api/prescriptions", {
    ...
    if rx13:
        sql(f"UPDATE prescriptions SET status='transmitted', prescribed_at='{days_ago(5)}', transmitted_at='{days_ago(4)}', transmission_method='dispensepro', ship_to='patient' WHERE id='{rx13}';")

    # Rx 2: Metformin 500mg — adjunct, shipped and delivered
    rx14 = api_post("/api/prescriptions", {
        "patient": f"/api/patients/{marcus}",
        "provider": f"/api/providers/{PROVIDERS['emily_martinez']}",
    ...
    if rx14:
        sql(f"UPDATE prescriptions SET status='shipped', prescribed_at='{days_ago(35)}', transmitted_at='{days_ago(34)}', filled_at='{days_ago(32)}', shipped_at='{days_ago(30)}', tracking_number='1Z999AA10123456784', carrier='UPS', transmission_method='unknown' WHERE id='{rx14}';")

    # Rx 3: Lisinopril — for mild HTN, transmitted
    rx15 = api_post("/api/prescriptions", {
        "patient": f"/api/patients/{marcus}",
        "provider": f"/api/providers/{PROVIDERS['emily_martinez']}",
    ...
    if rx15:
        sql(f"UPDATE prescriptions SET status='transmitted', prescribed_at='{days_ago(35)}', transmitted_at='{days_ago(34)}', transmission_method='scriptsure' WHERE id='{rx15}';")

    # Appointment: Monthly weight check — today, video
    apt6 = api_post("/api/appointments", {
        "patient": f"/api/patients/{marcus}",
        "provider": f"/api/providers/{PROVIDERS['emily_martinez']}",
        "clinic": f"/api/clinics/{CLINICS['westside']}",
...
if priya:
    ...
    rx16 = api_post("/api/prescriptions", {
    ...
    if rx16:
        sql(f"UPDATE prescriptions SET status='filled', prescribed_at='{days_ago(10)}', transmitted_at='{days_ago(9)}', filled_at='{days_ago(7)}', transmission_method='scriptsure' WHERE id='{rx16}';")

    # Rx 2: Levothyroxine — dose adjustment needed, draft
    rx17 = api_post("/api/prescriptions", {
        "patient": f"/api/patients/{priya}",
        "provider": f"/api/providers/{PROVIDERS['james_wilson']}",
        "medication": f"/api/medications/{MEDICATIONS['levothyroxine_50']}",
        "pharmacy": f"/api/pharmacies/{PHARMACIES['scriptsure']}",
    ...
    rx18 = api_post("/api/prescriptions", {
        "patient": f"/api/patients/{priya}",
        "provider": f"/api/providers/{PROVIDERS['james_wilson']}",
        "medication": f"/api/medications/{MEDICATIONS['metformin_500']}",
        "pharmacy": f"/api/pharmacies/{PHARMACIES['rxvortex']}",
    ...
    if rx18:
        sql(f"UPDATE prescriptions SET status='signed', prescribed_at=NOW(), signed_by_id='{PROVIDERS['james_wilson']}', signed_by_name='Dr. James Wilson' WHERE id='{rx18}';")

    # Appointment: Psychiatry follow-up tomorrow
    apt7 = api_post("/api/appointments", {
        "patient": f"/api/patients/{priya}",
        "provider": f"/api/providers/{PROVIDERS['sarah_johnson']}",
        "clinic": f"/api/clinics/{CLINICS['downtown']}",
    ...
    apt8 = api_post("/api/appointments", {
        "patient": f"/api/patients/{priya}",
        "provider": f"/api/providers/{PROVIDERS['james_wilson']}",
        "clinic": f"/api/clinics/{CLINICS['downtown']}",
