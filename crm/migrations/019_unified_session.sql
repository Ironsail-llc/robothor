...
--
-- Before: telegram:<chat_id> and agent:main:webchat-primary are separate sessions.
...
VALUES ('default', 'agent:main:primary', 'unified', 0)
...
SELECT
    (SELECT id FROM chat_sessions WHERE tenant_id = 'default' AND session_key = 'agent:main:primary'),
    m.message,
    m.created_at
...
WHERE s.tenant_id = 'default'
  ...
  AND NOT EXISTS (
      ...
      WHERE existing.session_id = (SELECT id FROM chat_sessions WHERE tenant_id = 'default' AND session_key = 'agent:main:primary')
...
SET message_count = (
    ...
    WHERE session_id = (SELECT id FROM chat_sessions WHERE tenant_id = 'default' AND session_key = 'agent:main:primary')
),
last_active_at = NOW()
