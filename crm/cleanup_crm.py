...
COMPANY_MERGE_CLUSTERS = [
    ...
    ("889e0da3-400b-4374-a80a-5526836e1a4d", "953a2fa9-5859-4d85-bafe-8b54664acae4", {}),
    # Thrive + Thrive Rx
    ("935f9bff-4ddd-4f47-9cdb-860c8bad8b04", "1aae39c3-d5a3-413b-9aca-6d7168c3f4dd", {}),
...
def _backup_data(conn) -> dict:
    ...
    for table in ("crm_people", "crm_companies", "crm_notes", "crm_tasks", "contact_identifiers"):
...
def _fix_data_quality(conn, dry_run: bool) -> int:
    ...
    for field in ("city", "job_title"):
        ...
        for row in rows:
            ...
            fixes += 1
            logger.info(
                "  Fix %s.%s: '%s' → '' (id: %s)", "crm_people", field, row[field], row["id"]
            )
...
def run_cleanup(dry_run: bool = True):
    ...
    if not backup_path.exists():
        ...
        logger.info(
            "Backup complete: %d people, %d companies",
    ...
    for keeper_id, loser_id, keeper_updates in COMPANY_MERGE_CLUSTERS:
        ...
        if not dry_run:
            ...
            if result:
                merged_companies += 1
                logger.info(
                    "  Merged company %s into %s (%s)", loser_id, keeper_id, keeper.get("name", "")
                )
            else:
            ...
            merged_companies += 1
            logger.info(
                "  Would merge company %s into %s (%s)", loser_id, keeper_id, keeper.get("name", "")
            )
    ...
    if not dry_run:
        ...
        logger.info(
            "Post-cleanup: %d people (-%d), %d companies (-%d)",
            post_people,
            pre_people - post_people,
            post_companies,
            pre_companies - post_companies,
        )
    else:
        logger.info(
            "DRY RUN complete. Would delete %d people, %d companies; "
